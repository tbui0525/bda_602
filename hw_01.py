...
def print_heading(title): #This was in your code Julien so I kept it here to make things pretty
    print("*" * 80)
    print(title)
    print("*" * 80)
    return
...
def diff_of_mean(iris, species, variable):
    ...
    fig.add_trace(
        go.Bar(x= [species, "not " + species], y = [len(spec), len(not_specs)],
    name = "Count of Each Species") ,secondary_y = False,

    )
    fig.add_trace(
        go.Scatter(x = [species, "not " + species], y = [np.mean(spec[variable]), np.mean(not_specs[variable])], name = "$\mu_0$"),
    secondary_y =True
    )
    fig.add_trace(
        go.Scatter(x = [species, "not " + species], y = [np.mean(iris[variable]), np.mean(iris[variable])], name = "$\mu_i - \mu_0$",
    ),secondary_y = True)
    ...
    fig.write_html(species + " " + variable + " difference of means.html", include_plotlyjs = "cdn")
    fig.show()
    return
def main():
    ...
    iris = px.data.iris()
    print(iris)
    # A lot of this code is just the same as the Titanic code but replaced the preprocessing type and dataset. Very similar to your slides
    ...
    fig1 = px.box(iris, x = iris["species"], color = iris["species"], y = iris["sepal_length"])
    fig1.write_html("Iris Box Plot.html", include_plotlyjs = "cdn")
    fig2 = px.violin(iris, x = iris["species"], color = iris["species"], y = iris["petal_length"])
    fig2.write_html("Iris Violin Plot.html", include_plotlyjs = "cdn")
    fig3 = px.histogram(iris, x= "sepal_length", y = "petal_length", color = "species", marginal = "box", hover_data = iris.columns)
    ...
    fig5 = px.density_heatmap(iris, x = "sepal_length", y = "sepal_width", marginal_x = "box", marginal_y = "box")
    ...
    test_df = pd.DataFrame.from_dict( #I just made up some random numbers with enough range to hopefully hit all the flower types
        [
            {"sepal_length": 3.4, "sepal_width": 1.2, "petal_length": 4.2, "petal_width": 1.1},
            {"sepal_length": 2.6, "sepal_width": 1.6, "petal_length": 1.4, "petal_width": 1.5},
            {"sepal_length": 1.4, "sepal_width": 0.6, "petal_length": 3.7, "petal_width": 0.8},
            {"sepal_length": 3.1, "sepal_width": 0.9, "petal_length": 2.8, "petal_width": 2.7}
            ]
    )
    ...
    for species in iris["species"].unique(): #Yes I know for loops are slow in python and nested for loops are even worse, but it was the best I could come up with
        for variable in iris.columns[:-2]: #Ignoring last 2 columns b/c that is species and speciesid
